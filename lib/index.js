...
SF.prototype.OrderService = function(data) {
  ...
  // data['things'] <=> data['cargo_list']
  if (data['things_num'] && data['things']) {
    xml += '<Cargo name="' + data['things'] + '" count="' + data['things_num'] + '"></Cargo>';
    data['cargo_list'] = [{
      name: data['things'],
      count: data['things_num']
    }];
  } else if (data['cargo_list'] && data['cargo_list'].length) {
    data['cargo_list'].forEach(cargo => {
      xml += '<Cargo name="' + cargo['name'] + '" count="' + cargo['count'] + '"></Cargo>';
    })
    data['things'] = _.map(data['cargo_list'], cargo => cargo.name).join(';');
    data['things_num'] = _.sum(_.map(data['cargo_list'], cargo => cargo.count));
  }
