...
SF.prototype.OrderSearchService = function(orderid) {
  ...
  const self = this
  return sfUtil.OrderSearchService(orderid).then(function(){ return self.Send(xml) })
}
...
SF.prototype.RouteService = function(mailno){
  ...
  const self = this
  return sfUtil.RouteService(mailno).then(function(){ return self.Send(xml) })
}
...
SF.prototype.RouteServiceByOrderId = function(orderid){
  ...
  xml += '<Head>' + this.checkHeader + '</Head>';
  xml += '<Body>';
  xml += '<RouteRequest tracking_type="2" method_type="1" tracking_number="' + orderid + '"/> ';
  xml += '</Body>';
  xml += '</Request>';

  const self = this
  return sfUtil.RouteServiceByOrderId(orderid).then(function(){ return self.Send(xml) })
}
